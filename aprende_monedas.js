...
// ============================================
// ASISTENTE  (SIN CAMBIOS — igual que en el original)
// ============================================
const preguntas = [
...
// ============================================================
// CONTENIDO NUEVO — animaciones para la página "Monedas"
// No tocan el asistente ni cambian ningún color/tamaño del
// diseño: solo agregan movimiento sutil a lo que ya existe.
// ============================================================

/* ---------------------------------------------------------
   1) REVELADO AL HACER SCROLL
   Marca como "reveal-on-scroll" cada bloque de contenido y
   cada tarjeta para que aparezcan con un fade + deslizamiento
   suave cuando entran en pantalla, en vez de estar todos
   visibles de golpe al cargar la página.
--------------------------------------------------------- */
function inicializarRevelado() {
    const elementosAAnimar = document.querySelectorAll(
        '.contenido-monedas .content-section, ' +
        '.contenido-monedas .info-card, ' +
        '.contenido-monedas .coin-card, ' +
        '.contenido-monedas .timeline-step'
    );

    if (!elementosAAnimar.length) return;

    elementosAAnimar.forEach(el => el.classList.add('reveal-on-scroll'));

    // Si el navegador no soporta IntersectionObserver, mostramos todo directo
    if (!('IntersectionObserver' in window)) {
        elementosAAnimar.forEach(el => el.classList.add('visible'));
        return;
    }
    ...
    const observador = new IntersectionObserver((entradas) => {
        entradas.forEach((entrada, indice) => {
            if (entrada.isIntersecting) {
                // Pequeño escalonado entre tarjetas de una misma fila
                // para que no aparezcan todas exactamente al mismo tiempo.
                const retraso = (indice % 4) * 80;
...
/* ---------------------------------------------------------
   2) "TINTINEO" DE MONEDA AL PASAR EL MOUSE
   Sobre cada tarjeta de denominación, la moneda gira levemente
   como si estuviera girando sobre la mesa. Es puramente visual,
   con CSS inline vía transform, así que no requiere tocar el
   diseño ni el CSS de la tarjeta en sí.
--------------------------------------------------------- */
function inicializarTintineoMonedas() {
    document.querySelectorAll('.coin-card').forEach(tarjeta => {
        const imagenMoneda = tarjeta.querySelector('img');
        if (!imagenMoneda) return;

        imagenMoneda.style.transition = 'transform 0.5s ease';

        tarjeta.addEventListener('mouseenter', () => {
            imagenMoneda.style.transform = 'rotateY(180deg)';
        });

        tarjeta.addEventListener('mouseleave', () => {
            imagenMoneda.style.transform = 'rotateY(0deg)';
        });
    });
}

/* ---------------------------------------------------------
   3) CONTADOR DE PROGRESO EN LA LÍNEA DE TIEMPO
   A medida que el usuario hace scroll dentro de la caja de
   "Breve historia de la moneda", va resaltando el paso
   correspondiente (1904 → Paridad → Cuara → Actualidad),
   dando sensación de recorrido histórico sin alterar el
   layout ya definido en el CSS.
--------------------------------------------------------- */
function inicializarLineaDeTiempoInteractiva() {
    ...
    const observadorPasos = new IntersectionObserver((entradas) => {
        entradas.forEach(entrada => {
            const icono = entrada.target.querySelector('.timeline-icon');
            if (!icono) return;

            if (entrada.isIntersecting) {
                icono.style.background = 'var(--primary-accent)';
                icono.style.color = 'var(--primary-dark)';
                icono.style.boxShadow = '0 0 0 6px rgba(212, 165, 116, 0.15)';
            } else {
                icono.style.background = '#10192a';
                icono.style.color = 'var(--primary-accent)';
                icono.style.boxShadow = 'none';
            }
        });
    }, { threshold: 0.6 });

    pasos.forEach(paso => observadorPasos.observe(paso));
}

document.addEventListener('DOMContentLoaded', () => {
    inicializarRevelado();
    inicializarTintineoMonedas();
    inicializarLineaDeTiempoInteractiva();
});